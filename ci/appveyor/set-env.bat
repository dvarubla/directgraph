...
set EXTRA_CONF_FLAGS=-DXPCOMPAT=ON -DIA32COMPAT=ON
shift
goto :loop
...
set EXTRA_CONF_FLAGS=-DXP64COMPAT=ON
shift
goto :loop
...
set CMAKE_CONFIGURE_FLAGS= ^
    ...
    %EXTRA_CONF_FLAGS% -DCMAKE_BUILD_TYPE=%CONFIGURATION% -DCMAKE_TOOLCHAIN_FILE=visual-c++_toolchain.txt
