        ...
        void BufferPreparer::prepareBuffer(IQueueReader *reader, uint_fast32_t offset, uint_fast32_t maxSize) {
            ...
            for (readIndex = offset; readIndex < size; readIndex++) {
                ...
                if (
                    ...
                    uint_fast8_t numStages = curDrawer->getNumStages();
                    if (!_drawItemProc.canCreateItems(numStages)) {
                        _canReadMore = false;
                        break;
                    }
                    if(numStages != 1) {
                        uint_fast32_t totalSize = curDrawer->getTotalSize() + _curUsedSize;
                        if (totalSize > maxSize) {
                            _canReadMore = false;
                            break;
                        }
                    }
                    for(uint_fast8_t stageNum = 0; stageNum < numStages; stageNum++) {
                        curDrawer->setStage(stageNum);
                        ...
                        if (isSemiTransparent) {
                            _isFirst =
                                    _drawItemProc.processStateDiff(_stateHelper.getCurItemState(), lastState,
                                                                   _transpBuffer.drawOps, true)
                        ...
                        if (isSemiTransparent) {
                            ...
                            if (!_isFirst) {
                                curDrawer->genDegenerates(curMem, _transpBuffer.prevCrds, crds.start,
                                                          _drawItemProc.getCurZ());
                            }
